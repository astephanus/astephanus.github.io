...
build_dir=$(mktemp -d -t build-XXXXXX)
...
touch .nojekyll
git init
git remote add myrepo "https://github.com/astephanus/astephanus.github.io.git"
git add -A
git commit -sm "GitHub pages update $(date)"
# Force updates in case of conflicts
git push -f --set-upstream myrepo main
